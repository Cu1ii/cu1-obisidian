...
- 当前 User 必须来自平台已经确认的登录状态。业务模块和客户端不得自行指定或切换 User 身份。
...
- 每条调用记录必须关联 User 和业务模块；存在会话时同时关联对应会话，以支持隔离、问题排查与成本核对。

## 5. 通用记忆生命周期

长期记忆点是基座管理的唯一通用记忆对象。它可以由业务模块基于对话或其他业务材料创建，并在不同状态间流转；处于“激活”状态的长期记忆点构成用户可使用的长期记忆。对话、阶段性总结等业务材料仅作为记忆点的来源引用，不构成另一类记忆对象。

### 5.1 基座职责

- 保存、查询、修改、删除长期记忆点，并执行其状态变更。
- 记录记忆点的 User 归属、业务模块范围、来源引用、版本、适用时间或情境及当前状态。
- 管理通用状态：待确认、激活、已拒绝、已停用和已删除。
- 保证只有处于激活状态且已获本次使用授权的记忆点，才能作为长期记忆参与后续处理。
- 业务模块范围默认隔离；跨业务模块使用记忆点属于未来演进事项。

### 5.2 与业务模块的边界

- 基座不判断记忆点的领域含义、证据是否充分或何时应当发起确认。
- 业务模块负责生成记忆点、决定业务分类和表达、说明依据与不确定性，并决定确认时机。
- 用户在业务模块中完成一次操作后，基座负责执行对应的通用记忆状态变更。
...
- 原始会话、长期记忆以及业务模块保存的心理分析结果在传输和存储过程中必须加密。
...
- 对话、上下文材料、业务分析结果、长期记忆点及其历史版本采用逻辑删除。
