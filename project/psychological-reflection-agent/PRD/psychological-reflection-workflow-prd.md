...
心理自我反思流程是运行在 [[business-workflow-runtime-prd|业务流程运行时]]中的领域流程扩展包。它定义心理自我反思的 WorkflowDefinition、领域 Skill 内容、领域规则和业务结果适配器；流程和 Skill 制品由运行时统一保存、版本化、发布、加载和观测。
...
- WorkflowDefinition 与 SkillDefinition 的保存、版本、发布、停用和隔离。
...
- 心理领域的 Prompt、Skill 内容和 description，供运行时发布和加载。

- 心理领域的 Prompt、Skill 内容和 description，供运行时发布和加载。??? 这个应该是交给运行时保管吧, 后台业务人员直接在运行时提供
- 心理观察的领域含义和确认规则。
- 心理模式的形成条件和复盘方式。
...
业务规则检查至少确认：依据来自用户本人明确提供的内容；事实、观察和可能解释彼此区分；存在不确定性和其他合理解释时已明确说明；长期模式具有多次支持记录并经用户确认；内容未越过本流程的禁止边界。
...
如果证据不足、现有材料粒度不足、存在多种无法区分的解释、处于激活状态的记忆点存在无法由时间或情境差异解释的实质冲突，或问题超出本流程的判断范围，复盘必须明确表示“目前无法判断”，不能为了生成结论而补足心理解释。
