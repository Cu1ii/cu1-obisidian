...
连接恢复采用「turn 记录 + 事件游标」双层方案：

1. Edge 接收请求时创建 turn 并落库。Worker 产生的可恢复输出按序写入 turn 记录，每个增量带单调递增的 `eventId`，再发布到热通道。
2. SSE 首次连接从当前 turn 的起点开始；重连请求携带 `Last-Event-ID`，Edge 按 session／turn 游标从热通道补发缺失事件，补发完成后再切换到实时订阅。
3. 若游标已经超出热通道保留窗口，Edge 直接读取 turn 记录生成最新快照，先恢复页面状态，再从快照对应的游标继续订阅。
4. Edge 重启或 Pod 滚更不影响 Worker；Worker 继续写 turn 记录和发布事件即可。事件补发与快照读取都按 `eventId` 去重，避免重连造成重复内容。

??? 1, 2, 3 的部分要详细一点, 最好从恢复场景来切入方案, 要不然理解起来很费劲
...
| 能力                | 用途                                                                                                               |
| ----------------- | ---------------------------------------------------------------------------------------------------------------- |
| `ChatClient` 统一接口 | 换供应商只改配置与 `ChatModel` bean，`foundation.model` 的路由与降级逻辑不受影响<br><br>??? 也就是说我还是要重启服务? 有没有什么热更新的办法, 因为后期可能会介入多家并行请求 |
...
| 僵尸 Execution 回收 | 每 5 分钟 | 进程崩溃、强制终止等异常场景的兜底恢复：超过 lease／超时阈值仍处 running 的置 failed，避免 session 永久卡在冲突态 |
...
僵尸 Execution 回收是执行状态的兜底任务，不是分布式锁的正常释放机制。Worker 正常完成、失败或取消时在 `finally` 中主动释放锁；Redis 锁设置 TTL／租约，进程异常时自动过期。扫描任务只处理确认 lease 已过期的长期 running Execution，并用 owner／token 校验后再推进状态，避免误删新持有者的锁。
??? 分布式锁不能这么取消吧, 宁愿用当前线程等待超时抛异常, 也不能定时任务搞吧, 涉及到跨线程取消分布式锁了, 引入的复杂度太高了 也没必要啊
...
工具调用属于 ReAct 的中间轮次，不直接作为用户可见正文。Worker 收到 Spring AI 的模型响应后，先校验工具名、参数和当前节点权限；通过校验才执行 `load_skill(skill_id)` 等受控工具，并将结果作为 tool message 回填上下文后再次调用模型。模型返回最终文本时，才交给流式或结构化输出处理。循环在 Worker 内受轮次、超时和 token 预算限制；校验失败或达到上限则进入失败／中断态并写入 trace。

??? 那我的 re-act 节点是怎么区分这次是不是流式返回呢, 类似的 agent 都是怎么实现的呢? 
...
同一 Execution 同时只允许一个写者，否则用户重复提交会产生两条并行推进的状态机。方案：数据库行锁或 Redis 锁；同一 session 存在进行中 turn 时，新提交返回冲突并携带当前 turnId，前端直接接上该流。
...
| 6   | 部分输出保留策略                      | 影响失败态展示与记忆点生成             | 按节点区分：P2 / P4 保留已生成内容并标记中断；P5 / P6 丢弃，避免半截总结被当作结论           |
...
**4 · turn 状态机与 SSE 事件表** — 一次用户提交的生命周期状态，以及推给前端的事件类型清单（token 增量、节点切换、闸门命中、结束、错误）。这是前后端能并行开工的契约：前端照事件表写渲染，后端照状态机写推进。
...
保留的输出在尾部追加不可编辑的中断标记，并记录 `interruptMode=active|passive`：主动包括用户／系统取消，被动包括超时、进程故障或依赖失败。
