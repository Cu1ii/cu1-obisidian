...
> **注意：上述规则不仅适用于 Mixed GC 自身 STW 内的晋升，也适用于两轮 Mixed GC 之间穿插的 Young GC 的晋升。** 
> 
> Mixed GC 的多轮之间应用在正常执行，Eden 满了就会触发普通 Young GC。这些 Young GC 也会把 Eden 存活对象晋升到 Old，**去向和 bitmap 规则完全一样**——晋升去空闲 Region、不进已有 Old Region、无 bitmap、本轮 Mixed GC 不收。  
> 
> 所以在日志里看到 `(mixed)` 之间有 `(young)` 穿插是正常的，它们也走空闲 Region，不影响本轮 CSet 选型。
