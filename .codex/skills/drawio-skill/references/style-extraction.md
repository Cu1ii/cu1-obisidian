...
1. Write the filled XML to `./preset-<name>-sample.drawio` in the user's working directory (not `/tmp/`, so the file is visible in the Obsidian vault).
2. Report the path to the user as an Obsidian wiki-link (e.g. `[[preset-<name>-sample.drawio]]`) — the user opens it via the Obsidian drawio plugin to review the rendering.
3. Show the user: preset summary table + sample file path + provenance/confidence line.

### Approval loop

- "save" / "looks good" → write candidate to `~/.drawio-skill/styles/<name>.json`; delete tempfile and sample `.drawio`.
- "change <field> to <value>" → edit the in-memory candidate; rewrite the sample `.drawio`; re-ask.
- "cancel" → delete tempfile and sample `.drawio`; no save.
...
**Prerequisite:** the agent's vision capability must be available (the agent reads the source image directly via its multimodal input). If vision is not available, stop and tell the user:
...
1. **Read the image.** Use the agent's vision input — read the source image file the user provided.
...
5. **Extract edge defaults.**
   - Right-angle orthogonal arrows → `edges.style = "edgeStyle=orthogonalEdgeStyle;rounded=1;orthogonalLoop=1;jettySize=auto;jumpStyle=arc;jumpSize=8;html=1"`.
