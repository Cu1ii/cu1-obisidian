...
4. **Write a sample `.drawio` file** using the sample-diagram skeleton in `references/style-extraction.md`, parameterized by the candidate preset. Save to `./preset-<name>-sample.drawio` in the user's working dir.
5. **Show the user:**
   - Preset summary table (palette hex values, shapes per role, font, edge style, extras).
   - The sample `.drawio` file path as an Obsidian wiki-link (e.g. `[[preset-<name>-sample.drawio]]`) — tell the user to open it in the Obsidian drawio plugin to review the rendering.
   - Provenance line: `source.type`, `source.path`, `extracted_at`, `confidence`.
6. **Wait for approval:**
   - "save" / "looks good" → write candidate to `~/.drawio-skill/styles/<name>.json`. Create `~/.drawio-skill/styles/` if it doesn't exist. Delete tempfile and sample `.drawio`.
   - "change `<field>` to `<value>`" → edit the in-memory candidate, rewrite the sample `.drawio`, re-ask.
   - "cancel" / "abort" / "no" → delete tempfile and sample `.drawio`; nothing saved.
...
| XML parse fails | Stop; report the parse error; suggest opening the file in the Obsidian drawio plugin to repair. |
